...
def logop(z):
  for word in z + '\n':
     ...
     time.sleep(0.01)
def maogit():
  os.system("xdg-open https://github.com/Angel-iluminaty/")
def ext():
  exit("\n{acl}[{rcl} !{acl} ] {gcl}GRACIAS POR USAR {acl}[{rcl} !{acl} ]\n".format(acl=acl,rcl=rcl,gcl=gcl))
def viewmail():
  if os.path.exists("core/domain.txt"):
    with open("core/domain.txt","r") as cmmail:
      cmmail = cmmail.read()
    print(f"{acl}>>>{bcl} CUREENT _ MAIL{acl} :{ycl} {cmmail}")  
    x_X=input(f"\n\n{ycl} VOLVER MENÚ PRINCIPAL {acl}[{gcl}y{ncl}/{rcl}n{acl}] {acl}:{bcl} ")
    if x_X=="y":
      main()
    elif x_X=="n":
      ext()  
...
def coustomnewdomains():
  ...
  email_name=str(input(f"{acl} >>>{gcl} INGRESE EL NOMBRE DE CORREO ELECTRÓNICO  {acl}:{ycl} "))
  for domainsrow,domains in enumerate(domain):
      print(f"{acl}[{gcl}",domainsrow+1,f"{acl}]{bcl}",domains["name"],f" {gcl}!{ncl}")
  ma=int(input(f"{acl}>>>{gcl} SELECCIONA TU DOMINIO {acl}:{ycl} "))
  if domainsrow+1 >= ma:
    inputdomain=ma-1
    print(f"{acl}[ {gcl}>>> {acl}]{gcl} SELECCIONA TU DOMINIO {acl}:{ycl}"+domain[inputdomain]["name"])
    ...
    if domaincon:
      logop(f"{acl}[{gcl} !{acl} ] {ycl}CREAR CON ÉXITO UN NUEVO CORREO ELECTRÓNICO {acl}[{gcl} !{acl} ]")
      logop("{acl}[{gcl} !{acl} ]{gcl} CORREO ELECTRÓNICO CREADO {acl}:{ycl} {newemail}".format(newemail=(json.loads(domainr.text))["email"],acl=acl,gcl=gcl,ycl=ycl))
    ...
    print(f"{acl}[{rcl} !{acl} ]{rcl} DOMINIO SELECCIONADO NO FUNDADO {acl}[{rcl} !{acl} ]")
...
def historylog():
  ...
  print(f"{acl}[{rcl} 0. {acl}]{pcl} VOLVER MENÚ PRINCIPAL {acl}[ {gcl}../{acl} ]")  
  in__=int(input(f"\n{bcl}>>>> {ycl}"))
  if num+1 >= in__ and in__ != 0:
    ...
    if login == historymail["email"]:
      logop(f"{acl}[{gcl} !{acl} ]{ycl} CORREO AÑADIDO EXITOSAMENTE {acl}[{gcl} !{acl} ]")
      inboxchkdef()
    else:
...
def randomemail():
  ...
  with open("core/domain.txt","w") as currend:
      ...
      currend.close()
  logop(f"{acl}[{gcl} !{acl} ] {ycl}CREAR CON ÉXITO UN NUEVO CORREO ELECTRÓNICO {acl}[{gcl} !{acl} ]")
  logop("{acl}[{gcl} !{acl} ]{gcl} CORREO ELECTRÓNICO CREADO {acl}:{ycl} {newemail}".format(newemail=randommail,acl=acl,gcl=gcl,ycl=ycl))    
  inboxchkdef()
      ...
      if str(defaultbox)==str(inboxget):
        ...
        if inboxget == []:
          ...
          if inboxget["message"]=="Email not found":
            os.system("clear;")
            print("{acl}[{rcl} !{acl} ] {ycl}TU CORREO ESTÁ VENCIDO {acl}[{rcl} !{acl} ]".format(acl=acl,rcl=rcl,ycl=ycl))
            time.sleep(2)
            main2()
            break
          else:
...
def more():
  ...
  {acl}[{gcl} 1{bcl}.{acl} ]{ycl} CREAR NUEVO CORREO{acl} [{ccl} RANDOM {acl}] 
  {acl}[{gcl} 2{bcl}.{acl} ]{ycl} CREAR NUEVO CORREO{acl} [{bcl} CREAT YOUR SELF {acl}]
  {acl}[{gcl} 3{bcl}.{acl} ]{ycl} VER TODO EL CORREO ELECTRÓNICO {acl}[{bcl} YOU CREATED {acl}]
  {acl}[{gcl} 4{bcl}.{acl} ]{ycl} VER CORREO ACTUAL {acl}[ {ccl}ALL MAILS {acl}]
  {acl}[{gcl} 5{bcl}.{acl} ]{ycl} HISTORIAL DE CORREOS ELECTRÓNICOS {acl}[ {bcl}YOU CAN LOG IN {acl}]
  {acl}[{gcl} 6{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Angel-iluminaty {acl}]
  ...
  try:
    ...
    x_X=input(f"\n\n{ycl} VOLVER AL MENU PRINCIPAL O TIPO {rcl}n{ycl} TO EXIT {acl}[{gcl}y{ncl}/{rcl}n{acl}] {acl}:{bcl} ")
    if x_X=="y":
      main2()
    elif x_X=="n":
      ext()      
def more2():
  ...
  {acl}[{gcl} 4{bcl}.{acl} ]{ycl} VER TODO EL CORREO ELECTRÓNICO {acl}[{bcl} YOU CREATED {acl}]
  {acl}[{gcl} 5{bcl}.{acl} ]{ycl} VER CORREO ACTUAL {acl}[ {ccl}ALL MAILS {acl}]
  {acl}[{gcl} 6{bcl}.{acl} ]{ycl} EMAILS HISTORY {acl}[ {bcl}YOU CAN LOG IN {acl}]
  {acl}[{gcl} 7{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Angel-iluminaty {acl}]
  {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT {acl}[{bcl} ×××{acl} ]
  """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
  try:
...
def main2():
  try:
    ...
    {acl}[{gcl} 5{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Angel-iluminaty {acl}]
...
def main():
  try:
    ...
    {acl}[{gcl} 1{bcl}.{acl} ]{ycl} CREAR NUEVO CORREO{acl} [{ccl} RANDOM {acl}] 
    {acl}[{gcl} 2{bcl}.{acl} ]{ycl} CREAR NUEVO CORREO{acl} [{bcl} CREAT YOUR SELF {acl}]
    {acl}[{gcl} 3{bcl}.{acl} ]{ycl} MÁS CARACTERÍSTICAS {acl}[ {bcl}... {acl}]
    {acl}[{gcl} 4{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Angel-iluminaty {acl}]
    {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT{acl} [{rcl} ××× {acl}]
    """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
    while True:
